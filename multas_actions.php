...
if (!function_exists('multa_processarTodas')) {
    function multa_processarTodas(mysqli $db): void {
        ...
        while ($row = $q->fetch_assoc()) {
            ...
            // Motivo padrão: "Não pagou a multa no prazo = Multa #X"
            $motivo = $db->real_escape_string("Não pagou a multa no prazo = Multa #{$id}");
            ...
            if ($chk && $chk->num_rows === 0) {
                ...
                // Já está na blacklist — atualiza o motivo para incluir esta multa
                $db->query("UPDATE blacklist SET motivo_texto='{$motivo}', tempo='permanente', expiracao=NULL WHERE rgpm='{$rgpm}'");
            }
        }
    }
}
...
if ($acao === 'aplicar_multa') {
    ...
    // Se meus_cursos estiver vazio, tenta pegar pelo pagamento pendente/validado
    $cursoAluno = trim($aluno['meus_cursos'] ?? '');
    if (!$cursoAluno) {
        $qCurso = $conexao->query("SELECT curso FROM pagamentos_pendentes WHERE usuario_id={$uid} ORDER BY id DESC LIMIT 1");
        if ($qCurso && $row = $qCurso->fetch_assoc()) {
            $cursoAluno = trim($row['curso'] ?? '');
        }
    }
    ...
    $curso = $conexao->real_escape_string($cursoAluno);
...
if ($acao === 'multa_validar') {
    ...
    // Garante que as colunas existem antes do INSERT
    ...
    // Registra o pagamento da multa no histórico
...
// ── EDITAR MULTA PENDENTE ─────────────────────────────────────────────────────
if ($acao === 'multa_editar') {
    exigirNivel(NIVEL_ADM);
    $id       = intval($_POST['id'] ?? 0);
    $valorStr = trim($_POST['valor'] ?? '0');
    $valor    = floatval(str_replace(',', '.', $valorStr));
    ...
    if ($valor <= 0) { echo json_encode(['sucesso' => false, 'erro' => 'Valor inválido']); exit; }
    if (!$linkAdv)   { echo json_encode(['sucesso' => false, 'erro' => 'Informe o link da advertência']); exit; }

    // Só permite editar multas ainda ativas e não vencidas
    $agora = date('Y-m-d H:i:s');
    $chk   = $conexao->query("SELECT id FROM multas WHERE id={$id} AND status='pendente' AND prazo_expira > '{$agora}' LIMIT 1");
    if (!$chk || $chk->num_rows === 0) {
        echo json_encode(['sucesso' => false, 'erro' => 'Multa não encontrada, já encerrada ou prazo vencido']);
        exit;
    }

    // Recalcula novo prazo a partir de agora
    $novoExpira = $prazoH === 0
        ? date('Y-m-d H:i:s', strtotime('+1 minute'))
        : date('Y-m-d H:i:s', strtotime("+{$prazoH} hours"));

    $ladv = $conexao->real_escape_string($linkAdv);
    $conexao->query("UPDATE multas SET valor={$valor}, link_adv='{$ladv}', prazo_horas={$prazoH}, prazo_expira='{$novoExpira}' WHERE id={$id}");

    echo $conexao->affected_rows >= 0
        ? json_encode(['sucesso' => true, 'nova_expira' => $novoExpira])
        : json_encode(['sucesso' => false, 'erro' => 'Erro ao atualizar: ' . $conexao->error]);
    exit;
}
...
// ── DELETAR LOG DE MULTA (apenas pagas) ──────────────────────────────────────
if ($acao === 'multa_log_deletar') {
    exigirNivel(NIVEL_ADM);
    $id = intval($_POST['id'] ?? 0);
    if (!$id) { echo json_encode(['sucesso' => false, 'erro' => 'ID inválido']); exit; }

    // Só permite deletar logs PAGOS
    $chk = $conexao->query("SELECT id, usuario_id FROM multas WHERE id={$id} AND status='paga' LIMIT 1");
    if (!$chk || $chk->num_rows === 0) {
        echo json_encode(['sucesso' => false, 'erro' => 'Apenas logs de multas pagas podem ser deletados']);
        exit;
    }
    ...
    if ($conexao->affected_rows > 0) {
        // Remove pagamento vinculado — garante que a coluna existe antes
        if ($uid) {
            $conexao->query("ALTER TABLE pagamentos_pendentes ADD COLUMN IF NOT EXISTS tipo_pagamento VARCHAR(20) DEFAULT 'taxa'");
...
if ($acao === 'multa_logs_apagar_todos') {
    exigirNivel(NIVEL_ADM);
    // Apenas logs pagos — nunca toca em nao_paga, blacklist ou status do usuário
    $res   = $conexao->query("SELECT COUNT(*) as t FROM multas WHERE status='paga'");
    $total = intval($res ? $res->fetch_assoc()['t'] : 0);
    $conexao->query("DELETE FROM pagamentos_pendentes WHERE tipo_pagamento='multa'");
    $conexao->query("DELETE FROM multas WHERE status='paga'");
    echo json_encode(['sucesso' => true, 'total' => $total]);
    exit;
}

// ── LIMPAR LOGS NÃO PAGOS (remove log + blacklist vinculada) ─────────────────
if ($acao === 'multa_limpar_nao_pagas') {
    exigirNivel(NIVEL_ADM);
    $res = $conexao->query("SELECT id, usuario_id, rgpm_aluno FROM multas WHERE status='nao_paga'");
    $total = 0;
    if ($res) {
        while ($row = $res->fetch_assoc()) {
            $uid2  = intval($row['usuario_id']);
            $rgpm2 = $conexao->real_escape_string($row['rgpm_aluno']);
            // Remove blacklist vinculada
            $conexao->query("DELETE FROM blacklist WHERE rgpm='{$rgpm2}'");
            // Desbloqueia usuário
            $conexao->query("UPDATE usuarios SET status='Pendente' WHERE id={$uid2} AND status='Bloqueado'");
            // Remove log
            $conexao->query("DELETE FROM multas WHERE id=" . intval($row['id']));
            $total++;
        }
    }
    echo json_encode(['sucesso' => true, 'total' => $total]);
...
if ($acao === 'multa_logs') {
    ...
    multa_processarTodas($conexao);
    $busca = trim($_POST['busca'] ?? '');
    $where = $busca !== '' ? "AND (nome_aluno LIKE '%" . $conexao->real_escape_string($busca) . "%' OR rgpm_aluno LIKE '%" . $conexao->real_escape_string($busca) . "%' OR discord_aluno LIKE '%" . $conexao->real_escape_string($busca) . "%')" : '';
